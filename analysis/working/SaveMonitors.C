...
//---Gate
int timeGate[2] = {-8, 2}; // min, max, 1 ch = 10 ns
...
Bool_t Monitors::Process(Long64_t entry)
{
   ...
   if (ProcessedEntries>NumEntries*Frac-1) {
    ...
    for (Int_t detID = 0; detID < numDet; detID++) {
      ...
      //=====================caculate array sili time stamp
      int asili = e_t[detID] - tac_t[1];
      ...
      //================ coincident with Recoil when z is calculated. ------added sili coincidence with recoil and array
      if( !TMath::IsNaN(z[detID]) ) { 
        for( int j = 0; j < 8 ; j++){
          if( TMath::IsNaN(rdt[j]) ) continue; 
	 
          int tdiff = rdt_t[j] - e_t[detID];
          int rsili = rdt_t[j] - tac_t[1];
      ...
      if( coinFlag && rdtgate ) {
        ...
        //filling EZ gated on Sili
        //if(asiligood) hezgasili->Fill( z[detID] , eCal[detID] );
        //if(rsiligood) hezgrsili->Fill( z[detID] , eCal[detID] );
