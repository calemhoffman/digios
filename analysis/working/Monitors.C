...
//TString rdtCutFile = "";
//TString rdtCutFile = "rdtCuts.30Si.root";
//TString rdtCutFile = "rdtCuts.31si.root";
TString rdtCutFile = "rdtCutsalwide.root"; // wide 29al gate
//TString rdtCutFile = "rdtCuts_recoilcheck.root"; // 29al gate for consistent checking of 29al/30si ratio (narrow)
//TString rdtCutFile = "rdtCuts.30si_recoilcheck.root"; // 30si gate for consistent checking of 29al/30si ratio
//TString rdtCutFile = "rdtCutsverywide.root";
//TString rdtCutFile = "rdtCuts.30Si_recoilcheck_1.root";	
//TString rdtCutFile = "rdtCutsSi.root";	
//TString rdtCutFile = "rdtCuts30Sinarrow.root";
//TString rdtCutFile = "rdtCutsSi29narrow.root";	
//TString rdtCutFile = "rdtCuts29Alnarrow.root";	
//TString rdtCutFile = "rdtCuts30Sideg.root";	
...
void Monitors::Begin(TTree *tree)
{
   ...
   */
   ...
   //for (Int_t i=0; i < 4; i++){
   //   htacRecoilsum[i] = new TH2F(Form("htacRecoilsum%d", i), Form("(RDT-%d+RDT-%d) - TAC; TAC ; Eres+dE ", i, i+1), 200, -2500 , -1000 , 200, 0, 8000);
   //}
   //===================== energy spectrum
...
Bool_t Monitors::Process(Long64_t entry)
{
   ...
   for( int i = 0; i < 8 ; i++){
      ...
      if( i % 2 == 0 /*&& tacGate[0] < tac[0] &&  tac[0] < tacGate[1]*/){
         ...
         if( isCutFileOpen ){
            cutG = (TCutG *)cutList->At(i/2) ;
            if(cutG->IsInside(rdt[i],rdt[i+1])){ // use the same cuts on tac-total energy 
