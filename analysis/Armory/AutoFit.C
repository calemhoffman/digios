...
Double_t nSkewGauss(Double_t *x, Double_t *par){
  Double_t result = 0;
  for( Int_t p = 0; p < nPeaks; p++){
    ...
    Double_t mean  = par[3*p+1];
    Double_t sigma = par[3*p+2];
    Double_t alpha = par[3*p+3]; // skewness
...
//########################################
//########################################
void fitGaussP2(TH1F * hist, double mean, double sigma, double xMin, double xMax, TString optStat = ""){
  
  
  if( gROOT->FindObjectAny("cFitGaussP2") == NULL ){
    TCanvas * cFitGaussP2 = new TCanvas("cFitGaussP2", "fit Gauss & P2", 800, 400);
  }
  gStyle->SetOptStat(optStat);
  
  hist->Draw();
  
  TF1 * fit = new TF1("fit", "[0] * TMath::Gaus(x, [1], [2], 1) + [3] + [4]*x + [5]*x*x", xMin, xMax);
  ...
  text.DrawLatex(0.15, 0.75,Form("count: %4.0f(%3.0f), E_{x}: %6.3f(%5.3f) MeV, #sigma: %3.0f(%3.0f) keV ", 
                                    ...
                                    paraA[1], paraE[1],
                                    paraA[2] * 1000., paraE[2] * 1000.));
  text.DrawLatex(0.15, 0.7, Form("P2: %4.2f(%3.2f) + %4.2f(%3.2f)x + %4.2f(%3.2f)x^{2}", 
                                    paraA[3],   paraE[3],
                                    paraA[4], paraE[4],
                                    paraA[5], paraE[5]));
  GoodnessofFit(hist, fit);
...
//########################################
//########################################
vector<double> fitAuto(TH1 * hist, int bgEst = 10, 
                       double peakThreshold = 0.1, 
                       double sigmaGuess = 20, 
                       int peakSeparation = 4, 
                       bool isSkewGauss = false, 
  ...
  if( isSkewGauss ) numParPerPeak = 4;
  ...
  for(int i = 0; i < nPeaks ; i++){
    ...
    para[numParPerPeak*i+2] = sigmaGuess;
    if( isSkewGauss ) para[numParPerPeak*i+3] = -3;
  }

  TF1 * fit;
  if( !isSkewGauss ){
    fit = new TF1("fit", nGauss, xMin, xMax, 3 * nPeaks );
  }else{
    fit = new TF1("fit", nSkewGauss, xMin, xMax, 4 * nPeaks );
  }
  fit->SetLineWidth(2);
  fit->SetLineColor(2);
  fit->SetNpx(1000);
  ...
  for( int i = 0; i < nPeaks; i++){
    ...
    fit->SetParLimits(numParPerPeak*i+2, 0, sigmaGuess * 5); 
    
    if( isSkewGauss ) fit->SetParLimits(numParPerPeak*i+3, -100, 0); 
  }
  ...
  */
  ...
  for(int i = 0; i < nPeaks ; i++){
    ...
    if( isSkewGauss ) ExAlpha[i] = paraA[numParPerPeak*i+3];
    if( newPlot){
      ...
      if( !isSkewGauss ){
        ...
        printf("%2d , count: %8.0f(%3.0f), mean: %8.4f(%8.4f), sigma: %8.4f(%8.4f), alpha: %4.1f(%4.1f) \n", 
            i, 
            paraA[numParPerPeak*i] / bw,   paraE[numParPerPeak*i] /bw, 
            paraA[numParPerPeak*i+1], paraE[numParPerPeak*i+1],
            paraA[numParPerPeak*i+2], paraE[numParPerPeak*i+2],
            paraA[numParPerPeak*i+3], paraE[numParPerPeak*i+3] );
  ...
  for( int i = 0; i < nPeaks; i++){
      if( !isSkewGauss ){
        ...
        gFit[i] = new TF1(Form("gFit%d", i), "[0] * TMath::Gaus(x,[1],[2], 1) * ( 1 + TMath::Erf( [3]*(x-[1])/sqrt(2)/[2] ))", xMin, xMax);
      }
      ...
      if( isSkewGauss ) gFit[i]->SetParameter(2, paraA[numParPerPeak*i+3]);
