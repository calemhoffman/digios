...
/**///======================================================== Ex and fit
   ...
   int * inX = new int[nPeakz];
   TMath::Sort(nPeakz, xpos, inX, 0 );
   vector<double> energy, height;
   for( int j = 0; j < nPeakz; j++){
      energy.push_back(xpos[inX[j]]);
      height.push_back(ypos[inX[j]]);
   }
   
   const int  n = 3 * nPeakz;
   double * para = new double[n]; 
   for(int i = 0; i < nPeakz ; i++){
      para[3*i+0] = height[i] * 0.05 * TMath::Sqrt(TMath::TwoPi());
      para[3*i+1] = energy[i];
      para[3*i+2] = 0.08;
   }

   TF1 * fit = new TF1("fit", fpeaks, ExRange[1], ExRange[2], 3* nPeakz );
   fit->SetLineWidth(1);
   fit->SetNpx(1000);
   ...
   double * ExPos = new double[nPeakz];
   double * ExSigma = new double[nPeakz];   
   for(int i = 0; i < nPeakz ; i++){
      ExPos[i] = paraA[3*i+1];
