...
void script_alpha() {
  ...
  cut[0]->SetPoint(0,    0.00, 835.81);
  cut[0]->SetPoint(1,  376.28, 545.69);
  cut[0]->SetPoint(2,  939.35, 428.07);
  cut[0]->SetPoint(3, 1478.95, 302.61);
  cut[0]->SetPoint(4, 1830.86, 192.83);
  cut[0]->SetPoint(5, 1760.48,   0.00);
  cut[0]->SetPoint(6,    0.00,   0.00);
  cut[0]->SetPoint(7,    0.00, 835.81);


  cut[1] = new TCutG("rdtCut1", 7);
  cut[1]->SetPoint(0,    0.00, 945.59);
  cut[1]->SetPoint(1,  274.62, 600.58);
  cut[1]->SetPoint(2,  618.71, 451.59);
  cut[1]->SetPoint(3, 1259.98, 255.56);
  cut[1]->SetPoint(4, 1408.56, 177.15);
  cut[1]->SetPoint(5, 1291.26,   0.00);
  cut[1]->SetPoint(6,    0.00,   0.00);
  cut[1]->SetPoint(7,    0.00, 945.59);


  cut[2] = new TCutG("rdtCut2", 5);
  cut[2]->SetPoint(0,    0.00, 992.64);
  cut[2]->SetPoint(1,  423.20, 616.26);
  cut[2]->SetPoint(2, 1486.77, 396.70);
  cut[2]->SetPoint(3, 1447.67,   0.00);
  cut[2]->SetPoint(4,    0.00,   0.00);
  cut[2]->SetPoint(5,    0.00, 992.64);


  cut[3] = new TCutG("rdtCut3", 5);
  cut[3]->SetPoint(0,    0.00, 1604.26);
  cut[3]->SetPoint(1,  321.54, 1212.19);
  cut[3]->SetPoint(2,  517.05,  867.18);
  cut[3]->SetPoint(3, 1510.23,  435.91);
  cut[3]->SetPoint(4, 1463.31,    0.00);
  cut[3]->SetPoint(5,    0.00,    0.00);
  cut[3]->SetPoint(6,    0.00, 1604.26);
  ...
  for (int i = 0; i < numDet; i++) {
    ...
    hEx_tCorr[i] = new TH2F(Form("hEx_tCorr%02d", i), Form("det-%02d (gate);tDiffFine [1 = 10 ns];Ex", i), 80, -30, 50, ExBin, ExRange[0], ExRange[1]);
    ...
    for (size_t k = 0; k < hitList.size(); k++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        if (cut[i]->IsInside(hitList[k].EE[i], hitList[k].dE[i])) {
          ...
          // if( hitList[k].detID == 18 ) {
          //   // if( hitList[k].nRDT > 1 ) hitList[k].PrintRDT();
          //   printf(" %d | %16.1f, %6.1f, %6.1f, %6.1f\n", hitList[k].nRDT, 
          //                             hitList[k].tDiff, 
          //                             hitList[k].te_t, 
          //                             hitList[k].rdtTraceTime, 
          //                             hitList[k].tDiffFineC);
          // }
