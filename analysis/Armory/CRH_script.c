...
tree->Draw(Form("e[%d]:x[%d]>>hex(500,-1.1,1.1,500,0,10)",i,i),"coinTime < 40 && x < 0 && CUTG","");
...
cc->Divide(1,2);
cc->cd(1);
tree->SetAlias("ep",Form("e[%d] - (x[%d]*%f + x[%d]*x[%d]*%f + x[%d]*x[%d]*x[%d]*%f)",i,i,p1,i,i,p2,i,i,i,p3));//[i-20][ab]));
// tree->SetAlias("ep",Form("e[%d] - (x[%d]*%f + x[%d]*x[%d]*%f+ x*[%d]*x[%d]*x[%d]*%f)",i,i,p1,i,i,p2,i,i,i,p3));
if (ab==0) tree->Draw(Form("ep*-1.+6>>hex(600,-1,11)"),"x < 0 && x > -0.95 && x < 0.95 && thetaCM>12","");
...
// Float_t pp0[10][2] = {{-1.010388,-1.021364},{-1.033074,-1.031451},{-1.050569,-1.062339},{-1.030345,-1.013438},{-1.003609,-1.096961}};
// Float_t pp1[10][2] = {{6.8577,6.8571},{8.0326,8.0742},{9.2145,9.2402},{10.1765,10.0251},{11.0830,11.6543}};
...
//     if (j==0) {
//         tree->Draw(Form("epp>>hEx%d%s",i,AB),"x < 0 && x > -1 && x < 1 && thetaCM>0","");
//     } else {
//         tree->Draw(Form("epp>>hEx%d%s",i,AB),"x > 0 && x > -1 && x < 1 && thetaCM>0","same");
//     }
//     hSum->Add(h[i+i*j]);
// }
// }
